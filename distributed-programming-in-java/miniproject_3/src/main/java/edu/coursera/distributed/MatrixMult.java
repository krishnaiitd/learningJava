...
public class MatrixMult {
     ...
     * matrix c.
     *
     * This method is called simultaneously by all MPI ranks in a running MPI program. For
     ...
     *
     * 1) The matrix a will only be filled with the input values on MPI rank zero. Matrix a on all
     ...
     *
     * 1) On rank zero, matrix c must be filled with the final output of the full matrix
     * multiplication. The contents of matrix c on all other ranks are ignored.
     *
     * Therefore, it is the responsibility of this method to distribute the input data in a and b
     * across all MPI ranks for maximal parallelism, perform the matrix multiply in parallel, and
     * finally collect the output data in c from all ranks back to the zeroth rank. You may use any
     * of the MPI APIs provided in the mpi object to accomplish this.
     *
     * A reference sequential implementation is provided below, demonstrating the use of the Matrix
     * class's APIs.
     *
     ...
     * @throws MPIException On MPI error. It is not expected that your implementation should throw
     * any MPI errors during normal operation.
     */
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        if (myRank == 0) {
            ...
            for (int i = 1; i < size; i++) {
                ...
                requests[i - 1] = mpi
                    .MPI_Irecv(c.getValues(), rowOffset, nElements, i, i, mpi.MPI_COMM_WORLD);
            ...
            mpi.MPI_Send(c.getValues(), start * c.getNCols(), (end - start) * c.getNCols(), 0,
