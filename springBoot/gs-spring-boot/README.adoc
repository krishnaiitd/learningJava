...
== How to Run this example for AOP check
Steps to compile and run it

* Go to the folder `gs-spring-boot`
* To compile
...
Check that 8080 port is available or not by `curl localhost:8080`, if available then run the jar [created by above command] on default port 8080
...
If 8080 port already occupied then add `-Dserver.port=8081` on any other port as below

[subs="attributes"]
----
$ mvn package && java -Dserver.port=8081 -jar target/gs-spring-boot-0.1.0.jar
----

Make a curl request to check that AOP is invoked:

[subs="attributes"]
----
$ curl localhost:8080/name/krishna
    Greeting from spring boot
----

And goes to the ternimal where the application is running for check the sysout, I have added the code class: `NullCheckAspect`
[subs="attributes"]
----
    Found the null checker annotation with name: nullChecker
    Found the null checker annotation with arg:  krishna
    {nullChecker=krishna}
    Before class: TimeTrackerAspect : String hello.HelloController.nullChecker(String)1553610780599
    krishna
    Total milli seconds in execution of method: String hello.HelloController.nullChecker(String) is :0
----