...
Check that 8080 port is available or not by `curl localhost:8080`, if available then run the below jar on default port 8080
...
If 8080 port already occupied then add on other port as below
...
And goes to the ternimal where the application is running for check the sysout
[subs="attributes"]
----
